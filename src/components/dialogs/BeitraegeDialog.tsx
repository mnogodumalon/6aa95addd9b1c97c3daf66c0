/**
 * BeitraegeDialog — pre-generated create/edit dialog for Beitraege.
 *
 ...
 * BeitraegeDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Beitraege['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<BeitraegeDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Beitraege, Mitglieder, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Beitraege';
...
/** Widened prefill type for BeitraegeDialog.defaultValues — see file header. */
export type BeitraegeDialogDefaults = Omit<Beitraege['fields'], 'status' | 'zahlungsart'> & {
    status?: LookupValue | string;
    zahlungsart?: LookupValue | string;
  };

interface BeitraegeDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Beitraege['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: BeitraegeDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  status: LOOKUP_OPTIONS['beitraege']?.['status'] ?? [],
  zahlungsart: LOOKUP_OPTIONS['beitraege']?.['zahlungsart'] ?? [],
};
...
export function BeitraegeDialog({ open, onClose, onSubmit, defaultValues, recordId, mitgliederList, enablePhotoScan = true, enablePhotoLocation = true }: BeitraegeDialogProps) {
  const [fields, setFields] = useState<Partial<Beitraege['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['jahr', 'betrag', 'faellig_am', 'status', 'mitglied'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Beitraege['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'beitraege');
      await onSubmit(clean as Beitraege['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "jahr": number | null, // Jahr\n  "betrag": number | null, // Betrag (€)\n  "faellig_am": string | null, // YYYY-MM-DD\n  "status": LookupValue | null, // Status (select one key: "offen" | "bezahlt" | "erlassen" | "gemahnt") mapping: offen=Offen, bezahlt=Bezahlt, erlassen=Erlassen, gemahnt=Gemahnt\n  "bezahlt_am": string | null, // YYYY-MM-DD\n  "zahlungsart": LookupValue | null, // Zahlungsart (select one key: "lastschrift" | "ueberweisung" | "bar") mapping: lastschrift=Lastschrift, ueberweisung=Überweisung, bar=Bar\n  "mitglied": string | null, // Display name from Mitglieder (see <available-records>)\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Beitraege['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('beitraege') })
    : t('new_entity', { entity: appLabel('beitraege') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'jahr': (
      <div key="jahr" className="space-y-1.5">
        <Label htmlFor="jahr">{fieldLabel('beitraege', 'jahr')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="jahr"
          type="number"
          inputMode="decimal"
        ...
        {showErrors && !fields.jahr && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('beitraege', 'jahr')}</p>
        )}
      </div>
    ),
    'betrag': (
      <div key="betrag" className="space-y-1.5">
        <Label htmlFor="betrag">{fieldLabel('beitraege', 'betrag')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="betrag"
          type="number"
          inputMode="decimal"
        ...
        {showErrors && !fields.betrag && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('beitraege', 'betrag')}</p>
        )}
      </div>
    ),
    'faellig_am': (
      <div key="faellig_am" className="space-y-1.5">
        ...
        <DatePicker
          id="faellig_am"
          placeholder="Fälligkeitsdatum setzen"
          mode="date"
          value={fields.faellig_am ?? null}
          onChange={v => setFields(f => ({ ...f, faellig_am: v ?? undefined }))}
          required
        />
        {showErrors && !fields.faellig_am && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('beitraege', 'faellig_am')}</p>
        )}
      </div>
    ),
    'status': (
      <div key="status" className="space-y-1.5">
        <Label htmlFor="status">{fieldLabel('beitraege', 'status')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('beitraege', 'status', 'offen') ?? 'Offen'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('beitraege', 'status', 'bezahlt') ?? 'Bezahlt'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'erlassen'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'erlassen' ? undefined : 'erlassen') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'erlassen'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('beitraege', 'status', 'erlassen') ?? 'Erlassen'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'gemahnt'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'gemahnt' ? undefined : 'gemahnt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'gemahnt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('beitraege', 'status', 'gemahnt') ?? 'Gemahnt'}
          </button>
        </div>
        {showErrors && !fields.status && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('beitraege', 'status')}</p>
        )}
      </div>
    ),
    'bezahlt_am': (
      <div key="bezahlt_am" className="space-y-1.5">
        <Label htmlFor="bezahlt_am">{fieldLabel('beitraege', 'bezahlt_am')}</Label>
        <DatePicker
          id="bezahlt_am"
          placeholder="Zahlungsdatum eintragen"
          mode="date"
    ...
    'zahlungsart': (
      <div key="zahlungsart" className="space-y-1.5">
        <Label htmlFor="zahlungsart">{fieldLabel('beitraege', 'zahlungsart')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('beitraege', 'zahlungsart', 'lastschrift') ?? 'Lastschrift'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('beitraege', 'zahlungsart', 'ueberweisung') ?? 'Überweisung'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('beitraege', 'zahlungsart', 'bar') ?? 'Bar'}
          </button>
        </div>
      </div>
    ),
    'mitglied': (
      <div key="mitglied" className="space-y-1.5">
        ...
        {showErrors && !fields.mitglied && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('beitraege', 'mitglied')}</p>
        )}
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"jahr": "Jahr", "betrag": "Betrag (€)", "faellig_am": "Fällig am", "status": "Status", "bezahlt_am": "Bezahlt am", "zahlungsart": "Zahlungsart", "mitglied": "Mitglied"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.BEITRAEGE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
