/**
 * HelferschichtenDialog — pre-generated create/edit dialog for Helferschichten.
 *
 ...
 * HelferschichtenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Helferschichten['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<HelferschichtenDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Helferschichten';
...
/** Widened prefill type for HelferschichtenDialog.defaultValues — see file header. */
export type HelferschichtenDialogDefaults = Helferschichten['fields'];

interface HelferschichtenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Helferschichten['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: HelferschichtenDialogDefaults;
...
export function HelferschichtenDialog({ open, onClose, onSubmit, defaultValues, recordId, veranstaltungenList, mitgliederList, enablePhotoScan = true, enablePhotoLocation = true }: HelferschichtenDialogProps) {
  const [fields, setFields] = useState<Partial<Helferschichten['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['bezeichnung', 'beginn', 'ende', 'benoetigte_helfer', 'veranstaltung'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Helferschichten['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'helferschichten');
      await onSubmit(clean as Helferschichten['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["veranstaltung", "helfer"]);
        ...
        const helferNames = raw['helfer'];
        if (Array.isArray(helferNames) && helferNames.length > 0) {
          const helferUrls = (helferNames as unknown[])
          ...
          if (helferUrls.length > 0) merged['helfer'] = helferUrls;
        }
        return merged as Partial<Helferschichten['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('helferschichten') })
    : t('new_entity', { entity: appLabel('helferschichten') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'bezeichnung': (
      <div key="bezeichnung" className="space-y-1.5">
        ...
        <Input
          id="bezeichnung"
          placeholder="z. B. Umkleidebetreuung"
          value={fields.bezeichnung ?? ''}
          onChange={e => setFields(f => ({ ...f, bezeichnung: e.target.value }))}
          required
        />
        {showErrors && !fields.bezeichnung && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('helferschichten', 'bezeichnung')}</p>
        )}
      </div>
    ),
    'beginn': (
      <div key="beginn" className="space-y-1.5">
        ...
        <DatePicker
          id="beginn"
          placeholder="Wann beginnt die Schicht?"
          mode="datetime"
        ...
        {showErrors && !fields.beginn && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('helferschichten', 'beginn')}</p>
        )}
      </div>
    ),
    'ende': (
      <div key="ende" className="space-y-1.5">
        ...
        {showErrors && !fields.ende && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('helferschichten', 'ende')}</p>
        )}
      </div>
    ),
    'benoetigte_helfer': (
      <div key="benoetigte_helfer" className="space-y-1.5">
        <Label htmlFor="benoetigte_helfer">{fieldLabel('helferschichten', 'benoetigte_helfer')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="benoetigte_helfer"
          type="number"
          inputMode="decimal"
          step="any"
        ...
        />
        {showErrors && !fields.benoetigte_helfer && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('helferschichten', 'benoetigte_helfer')}</p>
        )}
      </div>
    ),
    'bemerkung': (
      <div key="bemerkung" className="space-y-1.5">
        ...
        <Textarea
          id="bemerkung"
          placeholder="Besondere Aufgaben, Anforderungen..."
    ...
    'veranstaltung': (
      <div key="veranstaltung" className="space-y-1.5">
        ...
        {showErrors && !fields.veranstaltung && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('helferschichten', 'veranstaltung')}</p>
        )}
      </div>
    ),
    'helfer': (
      <div key="helfer" className="space-y-1.5">
        <Label htmlFor="helfer">{fieldLabel('helferschichten', 'helfer')}</Label>
        <MultiCombobox
          id="helfer"
          placeholder="Helfer eintragen"
          items={mitgliederListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.mitgliedsnummer ?? r.record_id),
          }))}
          values={extractRecordIds(fields.helfer)}
          onChange={ids => setFields(f => ({ ...f, helfer: ids.length ? ids.map(id => createRecordUrl(APP_IDS.MITGLIEDER, id)) as any : undefined }))}
          onCreateNew={(q) => openCreateMitglieder("helfer", q)}
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.HELFERSCHICHTEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
