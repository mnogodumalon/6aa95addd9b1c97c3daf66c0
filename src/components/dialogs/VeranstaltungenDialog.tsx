...
import type { Veranstaltungen, Abteilungen, LookupValue } from '@/types/app';
...
export type VeranstaltungenDialogDefaults = Omit<Veranstaltungen['fields'], 'status'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  status: LOOKUP_OPTIONS['veranstaltungen']?.['status'] ?? [],
};
...
export function VeranstaltungenDialog({ open, onClose, onSubmit, defaultValues, recordId, abteilungenList, enablePhotoScan = true, enablePhotoLocation = true }: VeranstaltungenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['titel', 'datum', 'ort', 'status'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "titel": string | null, // Titel\n  "beschreibung": string | null, // Beschreibung\n  "datum": string | null, // YYYY-MM-DDTHH:MM\n  "ort": string | null, // Ort\n  "maximale_teilnehmer": number | null, // Maximale Teilnehmer\n  "anmeldeschluss": string | null, // YYYY-MM-DD\n  "kostenbeitrag": number | null, // Kostenbeitrag (€)\n  "status": LookupValue | null, // Status (select one key: "geplant" | "offen_anmeldung" | "ausgebucht" | "abgesagt" | "durchgefuehrt") mapping: geplant=Geplant, offen_anmeldung=Offen für Anmeldung, ausgebucht=Ausgebucht, abgesagt=Abgesagt, durchgefuehrt=Durchgeführt\n  "abteilung": string | null, // Display name from Abteilungen (see <available-records>)\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'titel': (
      <div key="titel" className="space-y-1.5">
        ...
        {showErrors && !fields.titel && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('veranstaltungen', 'titel')}</p>
        )}
      </div>
    ),
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        ...
        <Textarea
          id="beschreibung"
          placeholder="Programm, Regeln, Besonderheiten..."
    ...
    'datum': (
      <div key="datum" className="space-y-1.5">
        ...
        {showErrors && !fields.datum && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('veranstaltungen', 'datum')}</p>
        )}
      </div>
    ),
    'ort': (
      <div key="ort" className="space-y-1.5">
        ...
        <Input
          id="ort"
          placeholder="z. B. Hallstadt, Turnhalle"
          value={fields.ort ?? ''}
          onChange={e => setFields(f => ({ ...f, ort: e.target.value }))}
          required
        />
        {showErrors && !fields.ort && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('veranstaltungen', 'ort')}</p>
        )}
      </div>
    ),
    'maximale_teilnehmer': (
      <div key="maximale_teilnehmer" className="space-y-1.5">
        <Label htmlFor="maximale_teilnehmer">{fieldLabel('veranstaltungen', 'maximale_teilnehmer')}</Label>
        <Input
    ...
    'anmeldeschluss': (
      <div key="anmeldeschluss" className="space-y-1.5">
        ...
        <DatePicker
          id="anmeldeschluss"
          placeholder="Anmeldeschluss wählen"
          mode="date"
    ...
    'kostenbeitrag': (
      <div key="kostenbeitrag" className="space-y-1.5">
        <Label htmlFor="kostenbeitrag">{fieldLabel('veranstaltungen', 'kostenbeitrag')}</Label>
        <Input
          id="kostenbeitrag"
          type="number"
          inputMode="decimal"
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        <Label htmlFor="status">{fieldLabel('veranstaltungen', 'status')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('veranstaltungen', 'status', 'geplant') ?? 'Geplant'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'offen_anmeldung'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'offen_anmeldung' ? undefined : 'offen_anmeldung') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'offen_anmeldung'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('veranstaltungen', 'status', 'offen_anmeldung') ?? 'Offen für Anmeldung'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'ausgebucht'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'ausgebucht' ? undefined : 'ausgebucht') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'ausgebucht'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('veranstaltungen', 'status', 'ausgebucht') ?? 'Ausgebucht'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('veranstaltungen', 'status', 'abgesagt') ?? 'Abgesagt'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('veranstaltungen', 'status', 'durchgefuehrt') ?? 'Durchgeführt'}
          </button>
        </div>
        {showErrors && !fields.status && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('veranstaltungen', 'status')}</p>
        )}
      </div>
    ),
    'abteilung': (
      <div key="abteilung" className="space-y-1.5">
        <Label htmlFor="abteilung">{fieldLabel('veranstaltungen', 'abteilung')}</Label>
        <Combobox
          id="abteilung"
  ...
  const FIELD_LABELS: Record<string, string> = {"titel": "Titel", "beschreibung": "Beschreibung", "datum": "Datum und Uhrzeit", "ort": "Ort", "maximale_teilnehmer": "Maximale Teilnehmer", "anmeldeschluss": "Anmeldeschluss", "kostenbeitrag": "Kostenbeitrag (€)", "status": "Status", "abteilung": "Abteilung (optional)"};
  const CURRENCY_KEYS = new Set<string>(["kostenbeitrag"]);
