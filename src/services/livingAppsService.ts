...
import type { Abteilungen, Mitglieder, Beitraege, Veranstaltungen, Anmeldungen, Helferschichten, CreateAbteilungen, CreateMitglieder, CreateBeitraege, CreateVeranstaltungen, CreateAnmeldungen, CreateHelferschichten } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getAbteilungen(): Promise<Abteilungen[]> {
    ...
    return hydrateRecords(records, 'abteilungen');
  }
  static async queryAbteilungen(q: RecordQuery = {}): Promise<Abteilungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.ABTEILUNGEN}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Abteilungen[];
  ...
  static async countAbteilungen(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.ABTEILUNGEN}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
  ...
  static async getAbteilungenEntry(id: string): Promise<Abteilungen | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Abteilungen;
    return hydrateRecords([record], 'abteilungen')[0];
  }
  static async createAbteilungenEntry(fields: CreateAbteilungen): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.ABTEILUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'abteilungen') });
    return { ...data, record_id: data.id };
  }
  static async updateAbteilungenEntry(id: string, fields: Partial<CreateAbteilungen>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.ABTEILUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'abteilungen') });
    return { ...data, record_id: data.id };
  }
  ...
  static async queryMitglieder(q: RecordQuery = {}): Promise<Mitglieder[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITGLIEDER}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Mitglieder[];
  ...
  static async countMitglieder(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITGLIEDER}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
    return parseAggregateCount(data);
  }
  ...
  // --- BEITRAEGE ---
  static async getBeitraege(): Promise<Beitraege[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BEITRAEGE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Beitraege[];
    return hydrateRecords(records, 'beitraege');
  }
  static async queryBeitraege(q: RecordQuery = {}): Promise<Beitraege[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BEITRAEGE}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Beitraege[];
  ...
  static async countBeitraege(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.BEITRAEGE}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
    return parseAggregateCount(data);
  }
  static async getBeitraegeEntry(id: string): Promise<Beitraege | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BEITRAEGE}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Beitraege;
    return hydrateRecords([record], 'beitraege')[0];
  }
  static async createBeitraegeEntry(fields: CreateBeitraege): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.BEITRAEGE}/records`, { fields: cleanFieldsForApi(fields as any, 'beitraege') });
    return { ...data, record_id: data.id };
  }
  static async updateBeitraegeEntry(id: string, fields: Partial<CreateBeitraege>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.BEITRAEGE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'beitraege') });
    return { ...data, record_id: data.id };
  }
  static async deleteBeitraegeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BEITRAEGE}/records/${id}`);
  }
  ...
  static async queryVeranstaltungen(q: RecordQuery = {}): Promise<Veranstaltungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.VERANSTALTUNGEN}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Veranstaltungen[];
  ...
  static async countVeranstaltungen(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.VERANSTALTUNGEN}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
    return parseAggregateCount(data);
  }
  ...
  // --- HELFERSCHICHTEN ---
  static async getHelferschichten(): Promise<Helferschichten[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.HELFERSCHICHTEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Helferschichten[];
    return hydrateRecords(records, 'helferschichten');
  }
  static async queryHelferschichten(q: RecordQuery = {}): Promise<Helferschichten[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.HELFERSCHICHTEN}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Helferschichten[];
  ...
  static async countHelferschichten(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.HELFERSCHICHTEN}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
    return parseAggregateCount(data);
  }
  static async getHelferschichtenEntry(id: string): Promise<Helferschichten | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.HELFERSCHICHTEN}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Helferschichten;
    return hydrateRecords([record], 'helferschichten')[0];
  }
  static async createHelferschichtenEntry(fields: CreateHelferschichten): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.HELFERSCHICHTEN}/records`, { fields: cleanFieldsForApi(fields as any, 'helferschichten') });
    return { ...data, record_id: data.id };
  }
  static async updateHelferschichtenEntry(id: string, fields: Partial<CreateHelferschichten>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.HELFERSCHICHTEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'helferschichten') });
    return { ...data, record_id: data.id };
  }
  static async deleteHelferschichtenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.HELFERSCHICHTEN}/records/${id}`);
  }
